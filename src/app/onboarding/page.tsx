...
export default function OnboardingPage() {
  ...
  return (
    <div
      className={`min-h-screen w-full bg-[#f4fff4] relative flex flex-col items-center pt-4 overflow-x-hidden ${splineSans}`}
    >
      {/* Header / Logo */}
      ...
      <main className="flex flex-col items-center w-full px-4 md:px-0 max-w-[1440px] mx-auto z-10">
        ...
        <div className="text-center mt-2 md:mt-4 mb-6 md:mb-8 max-w-[880px]">
          ...
          <p className="text-[#658670] text-[16px] md:text-[18px] font-normal max-w-[320px] md:max-w-none mx-auto">
            The next generation of social networking focused on genuine community and real-time
            experiences.
          </p>
        </div>
...
function FeatureCard({
  ...
  return (
    <div className="bg-white rounded-[24px] p-6 md:p-8 flex flex-col h-full min-h-[460px] shadow-sm hover:shadow-md transition-shadow duration-300 border-[0.8px] border-[#A3A3A3]">
      ...
      <div
        className={cn(
          'w-12 h-12 rounded-[12px] flex items-center justify-center mb-6',
          iconBg || 'bg-gray-100'
        )}
      >
      ...
      {/* Text Content */}
      <h3 className="text-[#121713] text-[24px] font-bold mb-3 leading-tight">{title}</h3>
      <p className="text-[#658670] text-[16px] leading-relaxed mb-4 flex-grow">{description}</p>
...
const CalendarIcon = ({ className }: { className?: string }) => <Calendar className={className} />;

const SignalIcon = ({ className }: { className?: string }) => <Radio className={className} />;
